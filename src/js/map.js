...
function initMap() {
    // GoogleMapAPIから現在地を取得する タイムアウトは10秒
    let options = {
        ...
        function callback(results, status) {
            if (status === google.maps.places.PlacesServiceStatus.OK) {
                ...
                for (let i = 0; i < results.length; i++) {
                    var json = {
                        "name": results[i].name,
                        "description": "星" + results[i].rating,
                        "latitude": results[i].geometry.location.toJSON().lat,
                        "longitude": results[i].geometry.location.toJSON().lng,
                        "holiday": 7
                    };
                    data.push(json);
                }
